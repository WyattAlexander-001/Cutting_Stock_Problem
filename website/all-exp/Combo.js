...
class Shelf {
  ...
  canFit(item) {
    // Only checks width since height check is handled in ShelfBin insert method
...
class ShelfBin {
  ...
  insert(item) {
    // Check existing shelves first
    for (let shelf of this.shelves) {
      // Try without rotation
      if (shelf.canFit(item)) {
        const newShelfHeight = Math.max(shelf.height, item.height);
        // Check if adding this item would exceed binHeight
        if (shelf.y + newShelfHeight <= this.binHeight) {
          shelf.addItem(item);
          this.items.push(item);
          return true;
        }
      ...
      if (this.allowRotation) {
        ...
        // Rotate back if unsuccessful
        item.rotate();
      }
    }

    // No existing shelf could fit the item, try to create a new shelf
    let shelfY = this.currentY + (this.shelves.length > 0 ? this.bladeThickness : 0);
    let requiredHeight = shelfY + item.height;

    // Try without rotation for a new shelf
    if (requiredHeight <= this.binHeight && item.width <= this.binWidth) {
    ...
    // Try with rotation for a new shelf if allowed
    if (this.allowRotation) {
      item.rotate();
